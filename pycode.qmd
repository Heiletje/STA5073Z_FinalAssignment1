...
```{python}
#---------------------------------------------------------------------------------------------------------------------------
# preliminaries: loading relevant libraries; importing and subsetting data for analysis
#---------------------------------------------------------------------------------------------------------------------------
# load libraries
import pandas as pd
...
data['cleaned_speech'] = data['speech'].apply(lambda text: re.sub(r'[^A-Za-z\s]', '', text).lower())

# select subset of four out of six presidents 
unique_presidents = data['president'].unique()
subset = data[~data['president'].isin(['Motlanthe', 'deKlerk'])]

# filtering data to remove stop words and only words with a length of three characters or more 
english_words = set(nltk.corpus.words.words())
stop_words = set(stopwords.words('english'))
# create a copy of the subset to avoid SettingWithCopyWarning
subset = subset[subset['president'].isin(unique_presidents)].copy()
subset['filtered_speech'] = subset['cleaned_speech'].apply(lambda text: ' '.join([word for word in text.split() if word not in stop_words and len(word) > 3 and word in english_words]))
#---------------------------------------------------------------------------------------------------------------------------
```
...
bow_features = bow_vectorizer.fit_transform(subset['filtered_speech']).toarray()

# create tf-idf ~ using 150 words 
tfidf_vectorizer = TfidfVectorizer(max_features=150)
tfidf_features = tfidf_vectorizer.fit_transform(subset['filtered_speech']).toarray()

# create embeddings
tokenized_speeches = [text.split() for text in subset['filtered_speech']]
model = Word2Vec(sentences=tokenized_speeches, vector_size=150, window=5, min_count=1, workers=4)
embeddings_features = np.array([np.mean([model.wv[word] for word in text.split() if word in model.wv] or [np.zeros(150)], axis=0) for text in subset['filtered_speech']])
#---------------------------------------------------------------------------------------------------------------------------
```
