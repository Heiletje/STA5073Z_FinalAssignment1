...
GGpresident_num_sentences_before <- ggplot(president_num_sentences, aes(x = president, y = num_sentences)) +
                                    ...
                                    axis.text.y = element_text(family = "Andale Mono", face="bold", size=11)) +
                                    guides(fill = guide_legend(nrow = 1))
GGpresident_num_sentences_before
...
# create a bar plot of presidents names and speech counts
GGpresident_num_sentences_after <- ggplot(president_num_sentences, aes(x = president, y = num_sentences)) +
                                    geom_bar(stat = "identity", fill = rdgy_palette[8]) +
                                    labs(x = "President", y = "Number of sentences") + 
                                    theme_bw() + theme(legend.box = "horizontal",
                                    panel.grid = element_blank(),
                                    ...
                                    axis.text.y = element_text(family = "Andale Mono", face="bold", size=11)) +
                                    guides(fill = guide_legend(nrow = 1))
GGpresident_num_sentences_after

```

```{r}
# calculate the average length of presidents' sentences (BEFORE removal of stop-words)
president_av_sentence_length <- tidySentences_sona %>% group_by(president) %>% select(president, sentence) %>%
                                summarize(av_sen_length = floor(mean(nchar(sentence)))) 
president_av_sentence_length$president <- factor(president_av_sentence_length$president, levels = c("deKlerk", "Mandela", "Mbeki", "Motlanthe", "Zuma", "Ramaphosa"))
...
# create a bar plot of presidents names and speech counts
ggplot(president_av_sentence_length, aes(x = president, y = av_sen_length)) +
...
# commented out to make rendering time more efficient
...
sona_sentences <- tidySentence_sona_final
