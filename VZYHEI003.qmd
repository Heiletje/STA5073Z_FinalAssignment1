...
#---------------------------------------------------------------------------------------------------------------------------------------------------------
# load packages
library(ggplot2) ; suppressMessages(library(dplyr)) ; suppressMessages(library(tidyverse)) ; library(tidytext) ; library(stringr) ; suppressMessages(library(lubridate)); library(textdata); suppressMessages(library(keras)); library(RColorBrewer)
...
# calculate the average length of presidents' sentences (after balancing data)
president_av_sentence_length_after <- tidySentences_sona4presidents_balanced %>% group_by(president) %>% select(president, sentence) %>%
                                       summarize(av_sen_length = floor(mean(nchar(sentence)))) 
president_av_sentence_length_after$president <- factor(president_av_sentence_length_after$president, levels = c("Mandela", "Mbeki", "Zuma", "Ramaphosa"))
...
president_av_sentence_length_after <- ggplot(president_av_sentence_length_after, aes(x = president, y = av_sen_length)) +
                                              ...
                                              legend.text = element_text(face="bold", size=6),
                                              legend.box.background = element_rect(colour = "black"),
                                              strip.text.x = element_text(family = "Andale Mono",size = 15, face = "bold"),
                                              plot.title = element_text(hjust = 0.5),
                                              axis.text.x = element_text(family = "Andale Mono", face="bold", size=11, angle = 55, hjust = 1),
                                              axis.text.y = element_text(family = "Andale Mono", face="bold", size=11)) +
                                              guides(fill = guide_legend(nrow = 1))
...
freq_words_allspeeches <- tidyWord_sona %>% count(word, sort = TRUE) %>%
                          ...
                          ggplot(aes(n, word)) + geom_col(fill = rdgy_palette[10]) +
...
freq_words_bypresident <- tidyWord_sona %>% count(president, word, sort = TRUE) %>%
                          ...
                          theme_bw() + theme(legend.box = "horizontal",
                                              ...
                                              plot.title = element_text(hjust = 0.5),
                                              axis.text.x = element_text(family = "Andale Mono", face="bold", size=9),
...
# use cleaned (e.g., removal of stop words, etc.) presidents' sentences for analysis purposes
# create BoW model ~ using 100 most frequent words for the four aforementioned presidents


# sample sentence due prior finding of imbalance in number of sentences
set.seed(5)
sona_sentences_balanced <- sona_sentences_4presidents %>% 
                           group_by(president) %>% slice_sample
