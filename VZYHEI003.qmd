...
#---------------------------------------------------------------------------------------------------------------------------------------------------------
# load packages
library(ggplot2) ; library(dplyr) ; library(tidyverse) ; library(tidytext) ; library(stringr) ; library(lubridate); library(textdata); library(keras)
...
# parse the date 
sona <-sona %>% mutate(date = str_sub(speech, start=1, end=30),
...
# token = unigram (one word)
tidyWord_sona <- sona %>% 
                 mutate(speech = str_replace_all(speech, replace_reg , ' ')) %>%    # remove any unwanted expression 
                 unnest_tokens(output = word, input = speech, token = "sent") %>%  # word tokenization 
                 filter(str_detect(word, "\\D"),                                    # retain only all non-digit words
                        !(nchar(word) < 2),                                         # remove words with excessively short length (< 2 letters)
                        !word %in% stop_words$word,                                 # removal of stop words
                        !word %in% c("madam", "madame", "madames"))                 # removal of common speech term in, misinterpreted as positive sentiment

# plot most frequent unigram (one word) used across all presidents 
tidyWord_sona %>% count(word, sort = TRUE) %>%
                  filter(n > 350) %>%
                  mutate(word = reorder(word, n)) %>%
                  ggplot(aes(n, word)) + geom_col() +
                  labs(y = "Most common word across all presidents' speeches", 
                       x = "Number of times word appears in speech") + 
                  theme_bw() + 
